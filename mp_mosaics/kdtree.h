...
class KDTree
{
  private:
    ...
    struct KDTreeNode
    {
      ...
      bool leafCheck() const {return !left && !right;}
    };
  ...
  private:
    ...
    KDTreeNode* constructor(int first, int last, int curDim, vector<Point<Dim>> & newPoints);
    void quickSelect(int k, int left, int right, int curDim, vector<Point<Dim>>& newPoints);
    KDTreeNode* copy(const KDTreeNode* cop);
    void del(KDTreeNode* curr);
    void nnsearch(const Point<Dim>& query, const KDTreeNode* subroot, Point<Dim>& currentBest, int curDim)const;
};
